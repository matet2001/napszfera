...
<li class="flex flex-col sm:flex-row py-4">
    <section class="relative w-full">
        ...
        <div class="w-full mx-auto px-2 sm:px-6 lg:px-0">
            <form action="{{ route('cart.remove', $item->product_id) }}" method="POST" class="absolute right-1 top-1 flex justify-center items-center">
                @csrf
                @method('DELETE')
            ...
            <div class="grid grid-cols-3 gap-4 sm:gap-8 mx-auto max-md:px-2">
                <div class="img rounded-xl">
                    <div class="img-box aspect-w-1 aspect-h-1 max-lg:mx-auto w-full">
                        <img src="{{ $item->image }}" alt="{{ $item->name }} image" class="h-full w-full object-cover object-center rounded-xl">
                    </div>
                </div>
                <div class="col-span-2 data w-full sm:pr-4 pr-0 justify-start flex flex-col sm:items-start items-center max-lg:pb-2">
                    <div class="data w-full max-w-xl text-left ml-3">
                        <a href="/termekek/{{ $item->product_id }}">
